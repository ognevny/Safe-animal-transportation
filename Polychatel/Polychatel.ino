...
void loop() {
  if (radio.available()) {  // если модуль получил информацию
    ...
    char* token = strtok(message.c_str(), " "); // начало парсинга строки
    int temperature = atoi(token); // парсим значение температуры
    token = strtok(NULL, " ");
    int humidity = atoi(token); // парсим значение влажности
    token = strtok(NULL, " ");
    int co2 = atoi(token); // парсим значение CO2
    token = strtok(NULL, " ");
    int regim = atoi(token); // парсим режим работы
    ...
    switch (regim) { // в зависимости от режима выводится различная информация на экран
      case 0: // стандартный режим: выводятся все данные
        lcd.setCursor(0, 0);
        lcd.print("Temp:");
        lcd.setCursor(8, 0);
        lcd.print("CO2:");
